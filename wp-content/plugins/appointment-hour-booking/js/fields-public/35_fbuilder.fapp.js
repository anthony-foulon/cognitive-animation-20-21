		    ...
		    if ((typeof specialPadding === 'undefined'))
		    {
		  	...
		  	for (var i=0;i<arr.length;i++)
		  	{
		  	  	...
		  	  	if (st >= et)
		  	        et += 24 * 60;  
		  	  	st += me.pb;
		  	  	while (st + me.duration + me.pa <=et  && st < 24 * 60)
		  	  	{ 
		...
		after_show:function()
		{
		    ...
		    getExtrasVisible = function(f)
		    {
		        try{
		            ...
		            for (var i=0;i<items.length;i++)
		                if (items[i].ftype=="fapp" && $("#"+items[i].name).parent().is(":visible"))
		                    getExtras(items[i],f)
		        } catch (e) {}        
		    }    
		    getExtras=function(me,f)
		    {
		        ...
		        if( e.length)
				{
				    find = true;
					e.each( function(){
					    if (($(this).is(":visible") || ($(this).prop("tagName")=="OPTION" && $(this).parent().is(":visible"))) &&  $.isNumeric(this.value))
						    v += this.value*1;
					});
				}
				me.percent = 0;
